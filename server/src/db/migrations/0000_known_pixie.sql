...
CREATE TABLE `memorization_test_sessions` (
	...
	`scope_week_number` int NOT NULL,
...
CREATE TABLE `weekly_training_records` (
	...
	CONSTRAINT `uq_user_week` UNIQUE(`user_id`,`week_number`)
);
--> statement-breakpoint
...
ALTER TABLE `weekly_training_records` ADD CONSTRAINT `weekly_training_records_user_id_users_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE no action ON UPDATE no action;