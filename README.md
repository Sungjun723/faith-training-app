...
# server/.env 파일을 열어 DB 접속 정보, JWT_SECRET, SMTP 정보를 실제 값으로 채운다.
# SMTP를 아직 설정하지 않았다면 비워둬도 되며, 이 경우 재설정 링크가 콘솔에 출력된다.
```
...
시드 스크립트 실행 후 콘솔에 출력되는 관리자 이메일/비밀번호로 로그인한다. 비밀번호는 로그인 화면의 "비밀번호를 잊으셨나요?"를 통해 이메일로 재설정할 수 있다 (SMTP 미설정 시에는 서버 콘솔에 재설정 링크가 출력된다).
...
4. **환경변수** (hPanel의 Node.js 앱 환경변수 설정 화면에서 등록, `.env` 파일을 커밋하지 않는다)
   ...
   - `JWT_SECRET`
   - `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM` (Hostinger 이메일 호스팅 정보 또는 외부 SMTP — 비밀번호 재설정 메일 발송용)
   - `APP_BASE_URL` (실제 배포 도메인, 예: `https://yourdomain.com` — 재설정 이메일의 링크 생성에 사용)
...
아래 항목은 코드 레벨 구현·타입체크·빌드까지 완료했지만, **실제 MySQL/SMTP 환경 연동 테스트는 배포 시 직접 진행이 필요**합니다 (샌드박스 환경에는 살아있는 DB가 없어 여기까지만 검증했습니다).
...
- 비밀번호 재설정은 구현되어 있으나, 실제 SMTP 연동 테스트는 배포 환경에서 직접 진행 필요
